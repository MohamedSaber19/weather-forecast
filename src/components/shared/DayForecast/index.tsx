...
const DayForecast: React.FC<Props> = ({ data }): JSX.Element => {
  const { icon, state, day_name, temp, dt_txt, humidity, wind_speed, temp_min, temp_max, feels_like } = data;
  ...
  return (
    <div className="hover:bg-teal-50 p-6 max-w-sm mx-auto bg-white rounded-xl shadow-lg flex flex-col">
      <div className="flex items-center justify-between pb-2">
        ...
        <div>
          <h2 className="text-xl font-medium text-black">{day_name}</h2>
          <h3 className="text-3xl text-teal-400 font-medium">{temp} &deg;C </h3>
